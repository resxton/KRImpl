...
# === Функции парсинга и отправки кадров ===
...
# === Класс GUI приложения с вкладками ===
class SerialAppTabs:
    def __init__(self, root):
        self.root = root
        self.root.title("UART GUI — Проект")
...
# === Основной класс панели подключения ===
class GUIModulePanel:
    def __init__(self, parent, is_sender=True):
        ...
        self.ser = None
        self.running = False
        self.my_addr = random.randint(0x40, 0x7E) if is_sender else random.randint(0x02, 0x3F)
    ...
    def create_widgets(self):
        ...
        # === Никнейм ===
        ttk.Label(self.parent, text="Никнейм:", style="Header.TLabel").grid(row=1, column=0, **grid_opts)
        self.nick_entry = ttk.Entry(self.parent)
        self.nick_entry.insert(0, self.nickname)
        self.nick_entry.grid(row=1, column=1, **grid_opts)
        ...
        ttk.Label(self.parent, text="Команды:", style="Header.TLabel").grid(row=2, column=0, **grid_opts)
        self.cmd_options = ['connect', 'disconnect', 'status', 'config', 'exit']
        self.cmd_var = tk.StringVar()
        self.cmd_combo = ttk.Combobox(self.parent, textvariable=self.cmd_var, values=self.cmd_options)
        self.cmd_combo.grid(row=2, column=1, **grid_opts)
        self.cmd_combo.bind("<<ComboboxSelected>>", self.on_command_selected)

        # === Адрес для connect ===
        self.addr_label = ttk.Label(self.parent, text="Адрес узла:", style="Header.TLabel")
        self.addr_entry = ttk.Entry(self.parent)
        self.addr_label.grid(row=3, column=0, **grid_opts)
        self.addr_entry.grid(row=3, column=1, **grid_opts)
        ...
        self.exec_btn.grid(row=3, column=2, **grid_opts)
        self.exec_btn.grid_remove()

        # === Сообщение ===
        if self.is_sender:
            ttk.Label(self.parent, text="Сообщение:", style="Header.TLabel").grid(row=4, column=0, **grid_opts)
            self.msg_entry = ttk.Entry(self.parent)
            self.msg_entry.grid(row=4, column=1, **grid_opts)
            self.send_msg_btn = ttk.Button(self.parent, text="Отправить", command=self.send_message)
            self.send_msg_btn.grid(row=4, column=2, **grid_opts)

        # === Лог ===
        self.log_text = scrolledtext.ScrolledText(self.parent, height=15, wrap=tk.WORD, state='disabled')
        self.log_text.grid(row=5 if self.is_sender else 4, column=0, columnspan=3, padx=10, pady=10, sticky='nsew')

        # Расширение по сетке
        for i in range(6 if self.is_sender else 5):
    ...
    def handle_command(self):
        ...
        if cmd == 'connect':
            ...
            if not self.connection:
                self.connection = Connection(self.my_addr, remote_addr, local_nick=self.nickname)
                try:
            ...
            self.log("Конфигурация порта не реализована в этом примере.", color="blue")
